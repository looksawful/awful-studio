...
MANIFEST = ROOT / 'assets/device_mockups/iphone_17/runtime/v20/iphone_17_v20.asset.json'
...
class DeviceDeliveryContractTests(unittest.TestCase):
    def test_v20_manifest_is_current_and_self_verifying(self):
        self.assertTrue(MANIFEST.is_file(), f'missing canonical v20 manifest: {MANIFEST}')
        ...
        self.assertEqual(manifest['version'], 'v20')
    ...
    def test_plugin_loader_matches_delivery_revision_and_stage(self):
        ...
        self.assertEqual(lod['variant'], 'low_v20')
        self.assertEqual(lod['source_revision'], manifest['source_revision'])
        self.assertEqual(Path(lod['blend_path']).name, 'iphone_17_low_v20.blend')
