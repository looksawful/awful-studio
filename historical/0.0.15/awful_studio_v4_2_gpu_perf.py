...
def mark_managed(datablock, role=""):
    try:
        datablock[MANAGED_KEY] = True
        datablock[ROLE_KEY] = role
        datablock[VERSION_KEY] = VERSION
    except Exception:
        pass
    return datablock
...
def remove_managed_studio():
    """Remove only AWFUL-managed scene data. Never delete arbitrary user objects."""
    ensure_object_mode()
    detach_external_children_from_managed()
    scene = bpy.context.scene

    if scene.world and is_managed(scene.world):
    ...
    # Blender 5.x stores the active compositor as a scene-owned node group.
    # Detach our managed graph first so it can actually be removed below.
    if hasattr(scene, "compositing_node_group"):
        try:
            comp_group = scene.compositing_node_group
            if comp_group and is_managed(comp_group):
    ...
    for collection in list(bpy.data.collections):
        if is_managed(collection):
            try:
                bpy.data.collections.remove(collection)
            except Exception:
                pass

    for group in (bpy.data.materials, bpy.data.curves, bpy.data.cameras, bpy.data.lights, bpy.data.meshes, bpy.data.node_groups):
        for datablock in list(group):
            if is_managed(datablock) and datablock.users == 0:
                try:
...
class StudioRegistry:
    ...
    def object(self, role):
        for current in bpy.data.objects:
            if current.get(ROLE_KEY) == role:
                return current
        return None

    def collection(self, role):
        for current in bpy.data.collections:
            if current.get(ROLE_KEY) == role:
                return current
        return None

    def material(self, role):
        for current in bpy.data.materials:
            if current.get(ROLE_KEY) == role:
                return current
        return None
...
def asset_root():
    path = bpy.utils.user_resource("DATAFILES", path="awful_studio/assets", create=True)
    if not path:
        path = os.path.join(bpy.app.tempdir, "awful_studio_assets")
...
def download_file(url, path, force=False):
    if file_is_valid(path) and not force:
        return True
    os.makedirs(os.path.dirname(path), exist_ok=True)
    tmp = path + ".part"
    request = urllib.request.Request(url, headers={"User-Agent": "AWFUL-Studio-Blender/4.0", "Accept": "*/*"})
    try:
        print(f"[AWFUL] downloading: {url}")
        with urllib.request.urlopen(request, timeout=120) as response, open(tmp, "wb") as output:
        ...
        print(f"[AWFUL] download failed: {url}\n  {exc}")
        try:
...
def ensure_assets(force=False):
    results = {}
    paint_zip = os.path.join(asset_root(), PAINT_ZIP_FILENAME)
    results["paint_zip"] = download_file(PAINT_ZIP_URL, paint_zip, force)
    results["paint_extract"] = extract_paint_bundle(paint_zip, force) if results["paint_zip"] else False
    for key, (_filename, url) in ASSET_URLS.items():
        results[key] = download_file(url, hdri_asset_path(key), force)
    return results
...
def get_or_create_managed_material(name, role):
    mat = REG.material(role) or bpy.data.materials.get(name)
    if mat is None:
...
def delete_object_hierarchy(root):
    """Delete an AWFUL-owned hierarchy bottom-up without leaving orphan children."""
...
class AWFUL_OT_FetchAssets(bpy.types.Operator):
    bl_idname="awful.fetch_assets_v4"; bl_label="Reload Assets"
    def execute(self,context):
        result=ensure_assets(True); refresh_world_images(); refresh_material_assets()
        self.report({"INFO"},f"Assets ready: {sum(bool(v) for v in result.values())}/{len(result)}")
        return {"FINISHED"}
...
class AWFUL_PT_Setup(bpy.types.Panel):
    ...
    def draw(self,context):
        ...
        col.operator("awful.build_studio_v4",icon="RECOVER_LAST")
...
LEGACY_CLASS_NAMES=("AWFUL_PT_StudioV2","AWFUL_PT_Main","AWFUL_PT_Lighting","AWFUL_PT_Product","AWFUL_PT_Camera","AWFUL_PT_Environment","AWFUL_PT_Assets","AWFUL_OT_BuildStudio","AWFUL_OT_UseSelectedProduct","AWFUL_OT_FetchAssets","AWFUL_OT_CyclePreset","AWFUL_StudioSettings")


def unregister_existing():
    try: del bpy.types.Scene.awful_studio
    except Exception: pass
    names=set(LEGACY_CLASS_NAMES)|{k.__name__ for k in CLASSES}
    for name in names:
    ...
    for k in reversed(CLASSES):
        try: bpy.utils.unregister_class(k)
        except Exception: pass


def register():
    global CAPS,REG
    unregister_existing(); CAPS=detect_blender_capabilities()
    for k in CLASSES: bpy.utils.register_class(k)
    bpy.types.Scene.awful_studio=PointerProperty(type=AWFUL_StudioSettings)
    REG=StudioRegistry(bpy.context.scene)
