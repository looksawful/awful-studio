...
## Historical-only ambientCG reference

Alpha 0.0.15 referenced ambientCG `PaintedPlaster017` (`https://ambientcg.com/view?id=PaintedPlaster017`) and its 2K PNG ZIP. The historical source remains byte-identical for migration evidence. The current Extension does not authorize that ZIP as an active network download. Its provenance remains recorded because the immutable historical fixture still contains the URL.
...
- A third-party binary asset must not be added to the Extension package without a separate explicit review and corresponding provenance/distribution change.
...
5. Run fast tests. `test_asset_provenance_contract.py` rejects unrecorded third-party `.hdr`/`.zip` URLs and bundled third-party binary media.
