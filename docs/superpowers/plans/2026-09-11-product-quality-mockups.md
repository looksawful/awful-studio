...
**Spec:** `docs/superpowers/specs/2026-09-11-studio-visual-product-completion-design.md`; product-material detail is refined by the authoritative Notion backlog row `Procedural mockup and material starter library`.
...
- Starter material acceptance includes matte plastic, glossy plastic, anodized metal, clear glass, dark glass, ceramic, cardboard, paper label, and screen/emissive where applicable.
...
- Starter keys: `PLASTIC_MATTE`, `PLASTIC_GLOSSY`, `METAL_ANODIZED`, `GLASS_CLEAR`, `GLASS_DARK`, `CERAMIC`, `CARDBOARD`, `PAPER_LABEL`, `SCREEN`.
...
def test_catalog_has_six_real_world_mockups():
    policy = load_policy()
    assert set(policy.mockup_keys()) == EXPECTED_MOCKUPS
    for key in EXPECTED_MOCKUPS:
        ...
        assert set(spec['material_slots']) == set(spec['slot_materials'])
        assert set(spec['slot_materials'].values()) <= EXPECTED_MATERIALS


def test_notion_material_starters_are_bounded_and_offline():
    policy = load_policy()
    assert set(policy.MATERIAL_STARTERS) == EXPECTED_MATERIALS
    assert policy.material_spec('GLASS_CLEAR')['transmission'] > 0.8
    assert policy.material_spec('GLASS_DARK')['transmission'] > 0.8
    assert policy.material_spec('METAL_ANODIZED')['metallic'] == 1.0
    assert policy.material_spec('SCREEN')['emission_strength'] > 0.0
```
...
MOCKUP_SPECS = {
    'BOTTLE': {
        ...
        'slot_materials': {'BODY': 'GLASS_CLEAR', 'CAP': 'PLASTIC_GLOSSY', 'LABEL': 'PAPER_LABEL'},
        'max_mesh_parts': 4, 'bevel_segments': 4,
    },
    'JAR': {
        'dimensions_m': (0.090, 0.090, 0.110),
        'material_slots': ('BODY', 'LID', 'LABEL'),
        'slot_materials': {'BODY': 'GLASS_CLEAR', 'LID': 'METAL_ANODIZED', 'LABEL': 'PAPER_LABEL'},
        'max_mesh_parts': 4, 'bevel_segments': 4,
    },
    'BOX': {
        'dimensions_m': (0.120, 0.070, 0.180),
        'material_slots': ('BODY', 'LABEL'),
        'slot_materials': {'BODY': 'CARDBOARD', 'LABEL': 'PAPER_LABEL'},
        'max_mesh_parts': 2, 'bevel_segments': 4,
    },
    'CAN': {
        'dimensions_m': (0.066, 0.066, 0.122),
        'material_slots': ('BODY', 'TOP', 'LABEL'),
        'slot_materials': {'BODY': 'METAL_ANODIZED', 'TOP': 'METAL_ANODIZED', 'LABEL': 'PAPER_LABEL'},
        'max_mesh_parts': 4, 'bevel_segments': 4,
    },
    'PHONE': {
        'dimensions_m': (0.071, 0.008, 0.147),
        'material_slots': ('FRAME', 'BACK', 'SCREEN'),
        'slot_materials': {'FRAME': 'METAL_ANODIZED', 'BACK': 'GLASS_DARK', 'SCREEN': 'SCREEN'},
        'max_mesh_parts': 4, 'bevel_segments': 5,
    },
    'TABLET': {
...
`MATERIAL_STARTERS` contains only numeric/shader intent. Required variants are matte/glossy plastic, anodized metal, clear/dark glass, ceramic, cardboard, paper label and screen. Glass uses bounded transmission/IOR; screen uses bounded emission. No texture/HDRI/network references are allowed.
...
Record source dimensions as custom properties on the root and assert bottom alignment before Auto Fit is approximately zero within `1e-4 m`. Verify material slot names/order against `material_slots` and material roles against `slot_materials`.
...
Each builder must mark objects and meshes through the current explicit scene ownership context and parent parts beneath one owned `MOCKUP_ROOT`. Bevel/subdivision segment counts come from `MOCKUP_SPECS`; no adaptive unbounded tessellation and no network calls.

- [ ] **Step 4: Implement starter materials once per scene owner**

Create/reuse owned materials by stable roles. Configure Principled BSDF with Blender 5.2 socket-name checks rather than assuming one historical socket layout. Material slot order on generated meshes must match `material_slots` metadata deterministically. `PLASTIC_MATTE` and `PLASTIC_GLOSSY` differ primarily in bounded roughness; `METAL_ANODIZED` is metallic; `GLASS_CLEAR`/`GLASS_DARK` use transmission; `CERAMIC`, `CARDBOARD`, `PAPER_LABEL`, and `SCREEN` keep bounded physically useful values.
...
- Adds registered operator `awful.generate_mockup` through the product-quality install hook/class list.
- Product panel shows `Mockup` selector + explicit `Generate / Replace AWFUL Mockup` action.
...
**Interfaces:**
- Consumes existing `legacy.mount_product`, camera framing policy, scene ownership and exact-ZIP verifier.
- Produces no new product API unless runtime evidence proves a gap.

- [ ] **Step 1: Add acceptance assertions before changing production code**
...
**Interfaces:**
- Produces review evidence in PR #36, GitHub issue #28 and the Notion backlog row.
