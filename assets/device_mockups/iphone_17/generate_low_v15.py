...
OUT = os.path.abspath(cli("--out", os.path.join(HERE, "generated", "iphone_17_low_v15.blend")))
EVIDENCE = os.path.abspath(cli("--evidence", os.path.join(HERE, "evidence", "low_v15_validation.json")))
PREVIEWS = os.path.abspath(cli("--previews", os.path.join(HERE, "previews", "low_v15")))
fc.clear_scene()
fc.setup_scene()
...
front_optic = fc.make_material("MAT_FRONT_OPTIC", (0.0045, 0.0065, 0.0110), 0.0, 0.026)
...
back_glass = fc.rounded_prism("BACK_GLASS", COVER_W, COVER_H, GLASS_T, COVER_R,
                              ...
                              edge_bevel=0.00008, outline_segments=48)
...
# Front camera / TrueDepth visual cluster from Apple dimensional drawing.
island_z = H*0.5 - 10.35*MM
island = fc.rounded_prism("DYNAMIC_ISLAND", 20.75*MM, 5.12*MM, 0.012*MM, 2.55*MM,
                          island_mat, detail_c, axis="Y",
                          location=(0, front_surface + 0.0015*MM, island_z), outline_segments=64)
# Hardware cutouts: a dark sensor capsule on the left and one round camera on the right, both on the deeper software island.
fc.rounded_prism("FRONT_SENSOR_PILL", 6.60*MM, 2.20*MM, 0.018*MM, 1.10*MM,
                 sensor_pill_mat, detail_c, axis="Y",
                 location=(-4.25*MM, front_surface + 0.0022*MM, island_z), outline_segments=48)
# Camera sits just above the black island surface, still inside the 7.95 mm envelope.
fc.cylinder("FRONT_CAMERA_GLASS", 1.12*MM, 0.022*MM, front_optic, detail_c,
            (5.08*MM, front_surface + 0.002*MM, island_z), axis="Y", vertices=96)
fc.cylinder("FRONT_CAMERA_INNER", 0.62*MM, 0.018*MM, black, detail_c,
            (5.08*MM, front_surface + 0.0015*MM, island_z), axis="Y", vertices=80)
fc.cylinder("FRONT_CAMERA_PUPIL", 0.27*MM, 0.014*MM, front_optic, detail_c,
            (5.08*MM, front_surface + 0.0015*MM, island_z), axis="Y", vertices=64)
receiver = fc.rounded_cube("FRONT_RECEIVER_MIC", (14.02*MM, 0.020*MM, 0.30*MM), 0.14*MM,
                           black, detail_c,
                           location=(0, front_surface + 0.008*MM, H*0.5 - 0.62*MM))

housing_x = -19.94*MM
housing_z = 52.32*MM
housing_seat = fc.rounded_prism("CAMERA_HOUSING_SEAT", 29.46*MM, 42.48*MM, 0.10*MM, 10.7*MM,
                                gap_mat, detail_c, axis="Y",
                                location=(housing_x, D*0.5 + 0.08*MM, housing_z), outline_segments=64)
housing_seat.hide_render = True
housing = fc.rounded_prism("CAMERA_HOUSING", 29.26*MM, 42.28*MM, 0.94*MM, 10.6*MM,
                           back_mat, detail_c, axis="Y",
                           location=(housing_x, D*0.5 + 0.54*MM, housing_z),
                           edge_bevel=0.00022, outline_segments=64)
for idx, (x_mm, z_mm) in enumerate(((-22.13, 61.18), (-22.13, 43.46)), 1):
    seat = fc.cylinder(f"CAMERA_{idx}_SEAT", 8.38*MM, 0.18*MM, gap_mat, detail_c,
                      (x_mm*MM, D*0.5 + 1.02*MM, z_mm*MM), axis="Y", vertices=160)
    seat.hide_render = True
    fc.cylinder(f"CAMERA_{idx}_RING", 8.00*MM, 0.64*MM, metal_dark, detail_c,
                (x_mm*MM, D*0.5 + 1.34*MM, z_mm*MM), axis="Y", vertices=160)
    fc.cylinder(f"CAMERA_{idx}_GLASS", 6.81*MM, 0.22*MM, lens_glass, detail_c,
                (x_mm*MM, D*0.5 + 1.77*MM, z_mm*MM), axis="Y", vertices=160)
    fc.cylinder(f"CAMERA_{idx}_INNER", 5.65*MM, 0.10*MM, black, detail_c,
                (x_mm*MM, D*0.5 + 1.94*MM, z_mm*MM), axis="Y", vertices=128)
    fc.cylinder(f"CAMERA_{idx}_PUPIL", 1.75*MM, 0.055*MM, lens_glass, detail_c,
                (x_mm*MM, D*0.5 + 2.020*MM, z_mm*MM), axis="Y", vertices=96)
fc.cylinder("FLASH_RING", 3.28*MM, 0.18*MM, gap_mat, detail_c,
            (-12.55*MM, D*0.5 + 1.03*MM, 52.25*MM), axis="Y", vertices=96).hide_render = True
fc.cylinder("FLASH", 3.14*MM, 0.18*MM, flash_mat, detail_c,
            (-12.55*MM, D*0.5 + 1.18*MM, 52.25*MM), axis="Y", vertices=96)
fc.cylinder("REAR_MIC", 0.50*MM, 0.18*MM, black, detail_c,
            (-12.65*MM, D*0.5 + 1.16*MM, 44.55*MM), axis="Y", vertices=64)

# Apple mark decal. Bounding box and vertical datum follow the Apple dimensional drawing.
logo_img_path = os.path.join(HERE, "reference", "apple_logo_alpha.png")
...
bsdf.inputs["Base Color"].default_value = (0.0025, 0.0030, 0.0040, 1.0)
...
root["asset_version"] = "low_v15_0.5"
...
root["apple_logo_decal"] = "reference/apple_logo_alpha.png"
...
mandatory = [
    ...
    "DYNAMIC_ISLAND", "FRONT_SENSOR_PILL", "FRONT_CAMERA_GLASS", "FRONT_CAMERA_PUPIL", "FRONT_RECEIVER_MIC", "APPLE_LOGO_DECAL",
...
evidence = {
    "asset_id": "iphone_17",
    "stage": "LOW_DRAFT",
    "revision": "front_sensor_pill_contract_v15",
...
fc.save_blend(OUT)
renders = (
    (cam_front, "iphone_17_low_v15_front.png"),
    (cam_back, "iphone_17_low_v15_back.png"),
    (cam_three, "iphone_17_low_v15_three_quarter.png"),
    (cam_left, "iphone_17_low_v15_left_side.png"),
    (cam_right, "iphone_17_low_v15_right_side.png"),
    (cam_bottom, "iphone_17_low_v15_bottom_macro.png"),
    (cam_screen, "iphone_17_low_v15_screen_edge_macro.png"),
    (cam_camera, "iphone_17_low_v15_camera_macro.png"),
    (cam_front_sensor, "iphone_17_low_v15_front_sensor_macro.png"),
    (cam_back_three, "iphone_17_low_v15_back_three_quarter.png"),
)
...
print("AWFUL_IPHONE17_V15_VALIDATION", json.dumps(evidence, sort_keys=True))
if not passed:
