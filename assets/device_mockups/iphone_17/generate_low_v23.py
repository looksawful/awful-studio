...
OUT = os.path.abspath(cli("--out", os.path.join(HERE, "generated", "iphone_17_low_v23.blend")))
EVIDENCE = os.path.abspath(cli("--evidence", os.path.join(HERE, "evidence", "low_v23_validation.json")))
PREVIEWS = os.path.abspath(cli("--previews", os.path.join(HERE, "previews", "low_v23")))
fc.clear_scene()
fc.setup_scene()
...
screen_bsdf.inputs["Emission Strength"].default_value = 2.4
...
ui_uv=ui_mesh.uv_layers.new(name="UVMap")
for loop,coord in zip(ui_mesh.loops,((0,0),(0,1),(1,1),(1,0))): ui_uv.data[loop.index].uv=coord
...
def physical_side_button(name, edge, z_mm, length_mm, face_width_mm=0.72, protrusion_mm=0.62, material=metal):
    # Mechanical buttons are proud of the rail. Keep the pocket shallow so the silhouette reads physically.
    cutter = fc.rounded_cube(f"{name}_CUTTER", (0.42*MM, (face_width_mm+0.34)*MM, (length_mm+0.54)*MM),
                             0.24*MM, None, detail_c)
    fc.place_on_rounded_edge(cutter, W, H, BODY_R, edge, z_mm*MM, outward=-0.16*MM, local_normal=(1,0,0))
    ...
    button = fc.rounded_cube(name, (thickness, face_width_mm*MM, length_mm*MM),
                             min(0.30*MM, face_width_mm*0.40*MM), material, detail_c)
    center_out = (protrusion_mm - thickness/MM*0.5) * MM
    fc.place_on_rounded_edge(button, W, H, BODY_R, edge, z_mm*MM, outward=center_out, local_normal=(1,0,0))
    return button

def camera_control(edge, z_mm, length_mm, face_width_mm=0.63):
    # Camera Control is a separate near-flush capacitive/mechanical surface, not a skinny side button.
    cutter = fc.rounded_cube("CAMERA_CONTROL_CUTTER", (0.34*MM, (face_width_mm+0.28)*MM, (length_mm+0.42)*MM),
                             0.20*MM, None, detail_c)
    fc.place_on_rounded_edge(cutter, W, H, BODY_R, edge, z_mm*MM, outward=-0.14*MM, local_normal=(1,0,0))
    fc.boolean_difference(body, cutter, name="CUT_CAMERA_CONTROL")
    boolean_cuts.append("CAMERA_CONTROL")
    control = fc.rounded_cube("CAMERA_CONTROL", (0.30*MM, face_width_mm*MM, length_mm*MM),
                              0.20*MM, metal_dark, detail_c)
    fc.place_on_rounded_edge(control, W, H, BODY_R, edge, z_mm*MM, outward=-0.03*MM, local_normal=(1,0,0))
    return control

# Apple drawing datums retained from v21; only the physical side-control profiles change.
physical_side_button("ACTION_BUTTON", "LEFT", 40.72, 11.6, 0.72)
...
camera_control("RIGHT", -23.40, 17.5, 0.63)
...
root["asset_version"] = "low_v23_0.8"
...
bm.free()
mandatory = [
    "BODY_ALUMINUM", "SCREEN_GLASS", "SCREEN_CONTENT", "SCREEN_UI_DECAL", "DISPLAY_BEZEL", "DISPLAY_GLASS_SEAT",
...
passed = (
    ...
    and camera_control_protrusion_mm <= 0.20
)
evidence = {
    "asset_id": "iphone_17",
    "stage": "LOW_DRAFT",
    "revision": "dimensional_screen_v23",
...
fc.save_blend(OUT)
renders = (
    (cam_front, "iphone_17_low_v23_front.png"),
    (cam_back, "iphone_17_low_v23_back.png"),
    (cam_three, "iphone_17_low_v23_three_quarter.png"),
    (cam_left, "iphone_17_low_v23_left_side.png"),
    (cam_right, "iphone_17_low_v23_right_side.png"),
    (cam_bottom, "iphone_17_low_v23_bottom_macro.png"),
    (cam_screen, "iphone_17_low_v23_screen_edge_macro.png"),
    (cam_camera, "iphone_17_low_v23_camera_macro.png"),
    (cam_front_sensor, "iphone_17_low_v23_front_sensor_macro.png"),
    (cam_back_three, "iphone_17_low_v23_back_three_quarter.png"),
)
